...
- **Provider adapters**
  ...
  - **Gemini** — `packages/coding-agent/src/web/search/providers/gemini.ts`
    ...
    - Model: `providers.webSearchGeminiModel` selects the Gemini grounding model; `GEMINI_SEARCH_MODEL` overrides it. Defaults to `gemini-2.5-flash`. OAuth providers resolve exact logical catalog ids to the selected provider's request model and report the logical id in results. With default effort, explicit wire ids and Developer API model ids pass through unchanged.
    - Effort: `providers.webSearchGeminiEffort` accepts `default`, `minimal`, `low`, `medium`, or `high`; `GEMINI_SEARCH_EFFORT` overrides it. `default` sends no `thinkingConfig`, preserving current and unknown models' native behavior. An explicit effort requires a logical model with verified catalog metadata: Antigravity uses its model-specific wire route and `thinkingBudget`, while Gemini CLI OAuth and the Developer API use `thinkingLevel` (with Gemini CLI wire routing where required). Unsupported efforts, unknown models, and raw wire ids fail before dispatch instead of being guessed. Thought summaries stay disabled.
